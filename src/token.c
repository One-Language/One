...
char* token_list_print_xml(token_list_t* list)
{
    ...
    if (list->size == 0) {
        ...
        string_append_format(str, "<tokens count=\"%zu\">\n", list->size);

        for (int i = 0; i < list->size; i++) {
            token_t* token = list->data[i];

            if (token->value == NULL) {
                string_append_format(str, "\t<token type=\"%s\" input=\"%.*s\" start_line=\"%d\" start_column=\"%d\" end_line=\"%d\" end_column=\"%d\"/>\n",
                ...
                string_append_format(str, "\t<token type=\"%s\" input=\"%.*s\" value=\"%s\" start_line=\"%d\" start_column=\"%d\" end_line=\"%d\" end_column=\"%d\"/>\n",
                                     token_name(token->type),
                                     (int)(token->end - token->start),
                                     token->start,
