...
void lexer_next(Lexer *lex) { lex->location.t++; }
...
void lexer_prev(Lexer *lex) { lex->location.t--; }
...
Token *lexer_getcurrent(Lexer *lex) { return lex->tokens[lex->location.t]; }

Token *lexer_get(Lexer *lex, size_t index) { return lex->tokens[index]; }
